...
<style lang="scss">
  ...
  .module {
    ...
    .name {
      ...
      position: relative;
      padding: 10px;
      text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8), /* основная темная тень */
      4px 4px 8px rgba(0, 0, 0, 0.6), /* глубокая тень */
      6px 6px 12px rgba(0, 0, 0, 0.4); /* еще глубже */
      ...
      &.highlight::before {
        ...
        width: calc(100% - 4px);
        height: calc(100% - 4px);
        border: solid 2px $clickgui-color-8;
      }

      &:hover {
        color: $clickgui-text-color;
        text-shadow: 3px 3px 6px rgba(0, 0, 0, 0.9), /* усиленная темная тень */
        5px 5px 10px rgba(0, 0, 0, 0.7), /* более глубокая тень */
        7px 7px 14px rgba(0, 0, 0, 0.5); /* самый глубокий слой */
      }

      &.enabled {
        background: linear-gradient(90deg, $clickgui-color-8, $clickgui-color-8);
        text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.9), /* усиленная темная тень для включенного состояния */
        4px 4px 8px rgba(0, 0, 0, 0.7); /* дополнительная тень для глубины */
        color: white;
      }
    }
    ...
    .settings {
      ...
      border-left: solid 4px $clickgui-color-8;
